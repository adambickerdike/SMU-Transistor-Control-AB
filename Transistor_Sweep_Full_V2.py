# -*- coding: utf-8 -*-
"""
Created on Mon Mar 17 22:04:55 2025
...
SMU_ADDRESS = 'USB0::0x0957::0x8E18::MY51143122::0::INSTR'  # Single SMU with two channels
...
# General settings
NPLC = 2.0                       # Integration time in power line cycles (0.1 to 10)
CURRENT_COMPLIANCE = 0.1         # Current compliance (A)
NUM_CYCLES = 40                 # Number of cycles
DELAY_BETWEEN_SWEEPS = 1.0       # Delay between sweeps (seconds)

# Device parameters
CHANNEL_LENGTH = 50e-6           # Channel length in meters
...
BASE_FILENAME = f"{SAVE_DIR}/Transistor_S2_1_MANA_40_CYCLE_{timestamp}"
...
def perform_output_sweep(smu, vg_fixed_list, cycle_num):
    ...
    # ===== MODIFIED OUTPUT DATA SAVE FORMAT =====
    # Save each Vg sweep to a separate CSV file with format similar to "Output ZnO S1 D11 04_03_2025 11_37_48.csv"
    current_date = datetime.now().strftime("%d_%m_%Y %H_%M_%S")
    
    # First, reorganize data to save each Vg in a separate file
    for vg in vg_fixed_list:
        # Filter data for this specific Vg
        vg_data = all_output_data[all_output_data['Vg_fixed'] == vg]
        
        # Get the required columns and sort by Vd
        output_data = vg_data[['Vd', 'Id', 'Ig']].copy()
        output_data = output_data.sort_values('Vd')
        
        # Save to file with new format
        output_filename = f"Output_S2_1_MANA_Vg{vg:.1f}V_Cycle{cycle_num}_{current_date}.csv"
        output_data.to_csv(f"{SAVE_DIR}/{output_filename}", index=False)
        print(f"Output data for Vg = {vg:.1f}V saved to {output_filename}")
    
    # Also save the combined data file as before
    ...
    print(f"All output data also saved to {BASE_FILENAME}_Output_AllVg_Cycle{cycle_num}.csv")
