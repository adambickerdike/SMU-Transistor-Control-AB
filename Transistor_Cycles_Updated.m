...
baseDir = 'C:\Users\bicke\Downloads\Transfer_40_Cycles\Transfer_40_Cycles\';  % Include trailing slash

% Base name and sweep type (split for easier modification)
baseName = 'PBS_Triple_ZnO_S2_D120250317_144531_Transfer';
sweepType = '_AllVd';
baseFileName = [baseName, sweepType]; % Combined to form complete base filename

% Parameters filename - adjust this based on your specific file naming pattern
parametersFileName = strrep(baseName, '_Transfer', '');
parametersFileName = [parametersFileName, '_Parameters_vs_Cycle.csv'];

% Colormap selection for transfer curve plots
% Options: 'cool', 'jet', 'parula', 'hot', 'turbo', 'spring', 'summer', 'autumn', 'winter', 'gray'
colormapName = 'summer';
...
paramFullPath = fullfile(baseDir, parametersFileName);
if exist(paramFullPath, 'file')
    try
        fprintf('Loading parameters file: %s\n', paramFullPath);
        paramsData = readtable(paramFullPath);
        hasParamsData = true;
...
% Define a colormap for the curves based on user selection
cmap = feval(colormapName, maxCycle);
...
if hasParamsData
    ...
    if saveOutput
        saveas(gcf, fullfile(baseDir, 'TransistorParameters.png'));
        saveas(gcf, fullfile(baseDir, 'TransistorParameters.fig'));
        fprintf('Parameters figures saved as %s\n', fullfile(baseDir, 'TransistorParameters.png'));
        fprintf('and %s\n', fullfile(baseDir, 'TransistorParameters.fig'));
    end
end
...
if saveOutput
    ...
    figure(1);
    saveas(gcf, fullfile(baseDir, 'TransistorTransferCurves.png'));
    saveas(gcf, fullfile(baseDir, 'TransistorTransferCurves.fig'));
    fprintf('Transfer curves figures saved as %s\n', fullfile(baseDir, 'TransistorTransferCurves.png'));
    fprintf('and %s\n', fullfile(baseDir, 'TransistorTransferCurves.fig'));
end
